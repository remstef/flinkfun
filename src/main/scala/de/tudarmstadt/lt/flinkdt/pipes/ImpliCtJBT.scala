...
object ImpliCtJBT {
  ...
  def exec_pipeline[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation](flip:Boolean, ct_raw:DataSet[CT2red[T1,T2]], all_marks:(T1, T2), env:ExecutionEnvironment): DataSet[CT2red[T1,T1]] = {
    ...
    val pipeline:mutable.ListBuffer[String] = mutable.ListBuffer.concat(DSTaskConfig.compute_pipeline)
      
    // BEGIN: compute checkpointed and pruned ct2
    val n11 = ct_raw
      .groupBy("a","b")
      .sum("n11")
      .checkpointed(DSTaskConfig.io_accumulated_AB, DSTaskConfig.jobname("(1) [N11Sum]"), DSTaskConfig.reread_checkpointed_data, env)(pipeline, "N11Sum")
