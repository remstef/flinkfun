...
object ImpliCtJBT {
  ...
  def exec_pipeline[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation](flip:Boolean, reread_checkpointed_data: Boolean, ct_raw:DataSet[CT2red[T1,T2]], all_mark_t1:T1, all_mark_t2:T2, env:ExecutionEnvironment): DataSet[CT2red[T1,T1]] = {
    ...
    val n11:DataSet[CT2ext[T1,T2]] = ct_raw
      .groupBy("a","b")
      .sum("n11")
      .checkpointed(DSTaskConfig.out_accumulated_AB, DSTaskConfig.jobname("(1) N11Sum"), reread_checkpointed_data, env)
    ...
    val n1dot = n11
      ...
      .checkpointed(DSTaskConfig.out_accumulated_A + suffix, DSTaskConfig.jobname("(3) [N1dotSum]" + suffix), reread_checkpointed_data, env)
    ...
    val ndot1 = n11
      ...
      .checkpointed(DSTaskConfig.out_accumulated_B + suffix, DSTaskConfig.jobname("(4) [Ndot1Sum]" + suffix), reread_checkpointed_data, env)
    ...
    val joined_n1dot = n11
      ...
      .checkpointed(DSTaskConfig.out_accumulated_CT + "_join_n1dot" + suffix, DSTaskConfig.jobname("(5.1) [Join N1dot]" + suffix), reread_checkpointed_data, env)
    ...
    val joined = joined_n1dot
      ...
      .checkpointed(DSTaskConfig.out_accumulated_CT + "_join_n1dot_ndot1" + suffix, DSTaskConfig.jobname("(5.2) [Join N1dot, Ndot1]" + suffix), reread_checkpointed_data, env)
    ...
    val ct2_complete = joined
      ...
      .map { _._1 }
      .checkpointed(DSTaskConfig.out_accumulated_CT + suffix, DSTaskConfig.jobname("(6) [Join N, Prune by LMI]" + suffix), reread_checkpointed_data, env)
    ...
    val ct_dt = ct2_complete
      ...
      .groupBy("a", "b")
      .sum("n11")
      .checkpointed(DSTaskConfig.out_dt + suffix, DSTaskConfig.jobname("(9) [DT: Sum]" + suffix), reread_checkpointed_data, env)

    val ct_dt_fsort = ct_dt
      .applyTask { FilterSortDT[CT2red[T1, T1], T1, T1](_.n11) }
      .checkpointed(DSTaskConfig.out_dt_sorted + suffix, DSTaskConfig.jobname("10 [DT: Filter, Sort]" + suffix), reread_checkpointed_data, env)
    // END: dt
