...
object ImpliCtJBT extends App {
  ...
  def exec_pipeline[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation](flip:Boolean, reread_checkpointed_data: Boolean, ct_raw:DataSet[CT2red[T1,T2]], all_mark_t1:T1, all_mark_t2:T2): DataSet[CT2red[T1,T1]] = {
    ...
    val n11:DataSet[CT2ext[T1,T2]] = ct_raw
      .groupBy("a","b")
      .sum("n11")
      .checkpointed(DSTaskConfig.out_accumulated_AB, _.toString, s => CtFromString[CT2red[T1, T2], T1, T2](s), DSTaskConfig.jobname("(1) N11Sum"), reread_checkpointed_data)
    ...
    val n = n11
      ...
      .checkpointed(DSTaskConfig.out_accumulated_N, _.toString, CtFromString[CT2ext[T1, T2], T1, T2](_), DSTaskConfig.jobname("(2) NSum"), reread_checkpointed_data)
    ...
    val n1dot = n11
      ...
      .checkpointed(DSTaskConfig.out_accumulated_A + suffix, _.toString, CtFromString[CT2ext[T1, T2], T1, T2](_), DSTaskConfig.jobname("(3) [N1dotSum]" + suffix), reread_checkpointed_data)
    ...
    val joined_n1dot = n11
      ...
      .checkpointed(DSTaskConfig.out_accumulated_CT + "_join_n1dot" + suffix, _.toString, CtFromString[CT2ext[T1, T2], T1, T2](_), DSTaskConfig.jobname("(5.1) [Join N1dot]" + suffix), reread_checkpointed_data)
    ...
    val joined = joined_n1dot
      ...
      .checkpointed(DSTaskConfig.out_accumulated_CT + "_join_n1dot_ndot1" + suffix, _.toString, CtFromString[CT2ext[T1, T2], T1, T2](_), DSTaskConfig.jobname("(5.2) [Join N1dot, Ndot1]" + suffix), reread_checkpointed_data)
    ...
    val ct_dt = ct2_complete
      ...
      .checkpointed(DSTaskConfig.out_dt + suffix + "__rawtemp", _.toString, s => CtFromString[CT2red[T1, T1], T1, T1](s), DSTaskConfig.jobname("(8) [DT: Join]" + suffix), reread_checkpointed_data)
      .groupBy("a", "b")
      .sum("n11")
      .checkpointed(DSTaskConfig.out_dt + suffix, _.toString, CtFromString[CT2red[T1, T1], T1, T1](_), DSTaskConfig.jobname("(9) [DT: Sum]" + suffix), reread_checkpointed_data)

    val ct_dt_fsort = ct_dt
      .applyTask { FilterSortDT[CT2red[T1, T1], T1, T1](_.n11) }
      .checkpointed(DSTaskConfig.out_dt_sorted + suffix, _.toString, CtFromString[CT2red[T1, T1], T1, T1](_), DSTaskConfig.jobname("10 [DT: Filter, Sort]" + suffix), reread_checkpointed_data)
    // END: dt
  ...
  exec_pipeline(false, DSTaskConfig.reread_checkpointed_data, env.readCT2r(DSTaskConfig.in_text), "*", "*").first(3).print
  
  exec_pipeline(true, DSTaskConfig.reread_checkpointed_data, env.readCT2r(DSTaskConfig.in_text), "*", "*").first(3).print
