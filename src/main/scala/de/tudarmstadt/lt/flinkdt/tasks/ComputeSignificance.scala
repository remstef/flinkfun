...
object ComputeSignificanceFiltered {
  ...
  def fromCT2Min[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation](sigfun:CT2def[_,_] => Float) = new SigFilter__from_CT2Min[T1,T2](sigfun)

  def fromCT2withPartialN[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation](sigfun:CT2def[_,_] => Float) = new SigFilter_from_CT2withPartialN[T1,T2](sigfun)
...
class Sig_from_CT2withPartialN[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation] extends DSTask[CT2def[T1,T2],(CT2def[T1,T2], Float)] {
  ...
  @transient
  val compute_ct:DSTask[CT2def[T1,T2], CT2def[T1,T2]] = ComputeCT2.fromCT2withPartialN[T1,T2]()

  override def fromLines(lineDS: DataSet[String]): DataSet[CT2def[T1,T2]] = lineDS.map(CtFromString[CT2def[T1,T2],T1,T2](_))

  override def process(cts: DataSet[CT2def[T1,T2]]): DataSet[(CT2def[T1,T2], Float)] = {
    compute_ct(cts)
      .map(ct => (ct, ct.lmi()))
  }
...
class Sig__from_CT2Min[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation] extends DSTask[CT2red[T1,T2],(CT2def[T1,T2], Float)] {
  ...
  @transient
  val chain:DSTask[CT2red[T1,T2], (CT2def[T1,T2], Float)] = N11Sum.toCT2withN[T1,T2]() ~>  ComputeSignificance.fromCT2withPartialN[T1,T2]()

  override def fromLines(lineDS: DataSet[String]): DataSet[CT2red[T1,T2]] = lineDS.map(CtFromString[CT2red[T1,T2], T1, T2](_))

  override def process(ds: DataSet[CT2red[T1,T2]]): DataSet[(CT2def[T1,T2], Float)] = {
    chain(ds)
  }
...
/**
  ...
  * @tparam T2
  */
class SigFilter_from_CT2withPartialN[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation](sigfun:CT2def[_,_] => Float) extends DSTask[CT2def[T1,T2],CT2def[T1,T2]] {
...
/**
  ...
  * @tparam T2
  */
class SigFilter__from_CT2Min[T1 : ClassTag : TypeInformation, T2 : ClassTag : TypeInformation](sigfun:CT2def[_,_] => Float) extends DSTask[CT2red[T1,T2],CT2def[T1,T2]] {

  @transient
  val chain:DSTask[CT2red[T1,T2], CT2def[T1,T2]] = N11Sum.toCT2withN[T1,T2]() ~>  ComputeSignificanceFiltered.fromCT2withPartialN[T1,T2](sigfun)
